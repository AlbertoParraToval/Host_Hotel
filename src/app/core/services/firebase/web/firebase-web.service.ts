...
import {
  ...
  FirebaseDocument,
  FirebaseService,
  FIRESTORAGE_PREFIX_PATH,
  FirestoreImages,
  FIRESTORE_IMAGES_COLLECTION,
...
export class FirebaseWebService extends FirebaseService implements OnDestroy {
  ...
  async init() {
    ...
    this.auth = initializeAuth(getApp(), {
  ...
  public fileUpload(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const path =
        FIRESTORAGE_PREFIX_PATH + '/' + prefix + '-' + Date.now() + extension;
        ...
        .then(async (snapshot) => {
          getDownloadURL(storageRef)
            .then(async (downloadURL) => {
              if (freeConnection) await signOut(this.auth);
              resolve({
                path,
                file: downloadURL,
              });
  ...
  public isUserConnectedAnonymously(): Promise<boolean> {
    const response = new Promise<boolean>(async (resolve, reject) => {
      resolve(
        this.auth.currentUser != null && this.auth.currentUser.isAnonymous
      );
    });
    return response;
  }
  ...
  public async connectAnonymously(): Promise<void> {
    const response = new Promise<void>(async (resolve, reject) => {
      ...
      else reject('An user is already connected');
    });
    return response;
  }
